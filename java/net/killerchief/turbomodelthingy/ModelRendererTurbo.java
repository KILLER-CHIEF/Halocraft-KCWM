...
public class ModelRendererTurbo extends ModelRenderer
{
    ...
    /**
     * Creates a trapezoid shape of any kind. (As similar to in Flan's Mod for Toolbox 2.0 Shape Compatibility)
     * @param xOrigin the starting x-position
     * @param yOrigin the starting y-position
     * @param zOrigin the starting z-position
     * @param width the width (over the x-direction)
     * @param height the height (over the y-direction)
     * @param depth the depth (over the z-direction)
     * @param scale the "scale" of the box. It only increases the size in each direction by that many.
     * @param xyz x0-7, y0-7, z0-7 coords for vector-like vertices.
     */
	public void addShapeBox(float xOrigin, float yOrigin, float zOrigin, int width, int height, int depth, float scale, 
	...
	{
		float xOpposite = xOrigin + (float)width;
		float yOpposite = yOrigin + (float)height;
		float zOpposite = zOrigin + (float)depth;
		xOrigin -= scale;
		yOrigin -= scale;
		zOrigin -= scale;
		xOpposite += scale;
		...
		if(this.mirror)
		{
			float xTemp = xOpposite;
			xOpposite = xOrigin;
			xOrigin = xTemp;
		}

		float[] v0 = {xOrigin - x0, yOrigin - y0, zOrigin - z0};
		float[] v1 = {xOpposite + x1, yOrigin - y1, zOrigin - z1};
		float[] v2 = {xOpposite + x5, yOpposite + y5, zOrigin - z5};
		float[] v3 = {xOrigin - x4, yOpposite + y4, zOrigin - z4};
		float[] v4 = {xOrigin - x3, yOrigin - y3, zOpposite + z3};
		float[] v5 = {xOpposite + x2, yOrigin - y2, zOpposite + z2};
		float[] v6 = {xOpposite + x6, yOpposite + y6, zOpposite + z6};
		float[] v7 = {xOrigin - x7, yOpposite + y7, zOpposite + z7};

		addRectShape(v0, v1, v2, v3, v4, v5, v6, v7, width, height, depth);
	}
    	...
    	else
    	{
    		...
    		for(int i = 0; itr.hasNext(); i++)
    		{
    			...
    			if(!defaultTexture.equals("")){
    				field_110782_f = new ResourceLocation(defaultTexture);
    				renderEngine.bindTexture(field_110782_f);//renderEngine.bindTexture(renderEngine.getTexture(defaultTexture));
    			}
    		}
    	}
    }
