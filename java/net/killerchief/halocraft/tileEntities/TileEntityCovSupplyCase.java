...
public class TileEntityCovSupplyCase extends TileEntity implements IInventory {

	public ItemStack ContainerItemStacks[];

	public TileEntityCovSupplyCase()
	{
		this.ContainerItemStacks = new ItemStack[20];
	}
		...
		if (!this.worldObj.isRemote)
		{
			if (this.tickNext < System.currentTimeMillis())
			{
				...
				for (int p : new int[]{1,3}) {
					if (this.ContainerItemStacks[p] != null && this.ContainerItemStacks[p].getItem() == HalocraftItems.PlasmaPowerCore)
					{
						for (int i : new int[]{4+(p==3?8:0),5+(p==3?8:0)}) {
							if (this.ContainerItemStacks[i] != null)
							{
								if (this.ContainerItemStacks[i].getItem() == HalocraftItems.SwordHilt) {
									if (this.ContainerItemStacks[i].isItemDamaged())
									{
										...
										this.ContainerItemStacks[p].setItemDamage(this.ContainerItemStacks[p].getItemDamage()+1);
										//break;
									}
								}
	...
	{
		if (this.ContainerItemStacks != null && this.ContainerItemStacks.length >= 2)
		{
			if (front) {
				if (this.ContainerItemStacks[0] != null)
					return this.ContainerItemStacks[0].getItem() == HalocraftItems.CovArmorPlate;
			} else {
				if (this.ContainerItemStacks[2] != null)
					return this.ContainerItemStacks[2].getItem() == HalocraftItems.CovArmorPlate;
			}
		}
		return false;
	}
	...
	{
		return "container.halocraft.covsupplycase";
	}
