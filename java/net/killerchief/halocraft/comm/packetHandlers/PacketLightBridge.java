...
public class PacketLightBridge implements IMessage {
	...
	public PacketLightBridge() { }

	public PacketLightBridge(int x, int y, int z, byte btnval, byte offset) {
		this.X = x;
		this.Y = y;
		this.Z = z;
		this.btnVal = btnval;
	...
	public static class Handler implements IMessageHandler<PacketLightBridge, IMessage>
	{
		@Override
		public IMessage onMessage(PacketLightBridge message, MessageContext ctx)
		{
			if (ctx.side == Side.SERVER) {
				EntityPlayerMP player = ctx.getServerHandler().playerEntity;
				if (player.worldObj.getTileEntity(message.X, message.Y, message.Z) instanceof TileEntityLightBridgeGen)
				{
					TileEntityLightBridgeGen tileentity = ((TileEntityLightBridgeGen)player.worldObj.getTileEntity(message.X, message.Y, message.Z));
					...
					if (tileentity.getUsername().equalsIgnoreCase("") || tileentity.getUsername().equalsIgnoreCase(player.getDisplayName()))
					{
						...
						if (message.Offset != 0)
						{
							tileentity.offsetBridgeDistance(message.Offset);
						}
					}
